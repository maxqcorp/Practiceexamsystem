...
export default function Exam() {
  ...
  const examSet = examSets.find(s => s.id === Number(setId));
  const [answers, setAnswers] = useState<Map<number, number>>(new Map());

  useEffect(() => {
    if (examSet) getExamSetProgress(examSet.id).then(setAnswers);
  }, [examSet?.id]);

  useEffect(() => {
    if (examSet && answers.size > 0) saveExamSetProgressDebounced(examSet.id, answers);
  }, [answers, examSet?.id]);

  if (!examSet) return (
    <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center">
      <div className="text-center"><p className="text-gray-500 mb-4">Set not found.</p>
        <Link to="/sets-50"><Button>Back to Sets</Button></Link></div>
    </div>
  );
  ...
  return (
    <ExamView
      ...
      onReset={async () => { setAnswers(new Map()); await clearExamSetProgress(examSet.id); }}
      backLink="/sets-50"
      backLabel="DumpsGate Standard"
      accentClass="indigo"
      modeStorageKey="exam-mode-main50"
    />
  );
}
