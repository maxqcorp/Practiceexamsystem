...
export default function QuickPractice() {
  ...
  const filteredSets = examSets25.filter(set => {
  ...
  const continueSet = examSets25.find(set => {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Reset all DumpsGate Quick sets?</AlertDialogTitle>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2.5 mb-1">
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-700 border border-purple-200">DumpsGate Quick</span>
          </div>
          <h1 className="text-xl font-semibold text-gray-900">DumpsGate Quick Practice</h1>
          <p className="text-sm text-gray-400 mt-0.5">{totalQuestions.toLocaleString()} questions · {examSets25.length} sets</p>
        </div>
        ...
        {continueSet && (() => {
          ...
          return (
            <Link to={`/quick-practice/${continueSet.id}`}>
        ...
        {filteredSets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredSets.map(set => {
              ...
              return (
                <div key={set.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-3 hover:border-purple-300 hover:shadow-sm transition-all">
                  {/* Set header */}
                  ...
                  <div className="flex flex-col gap-1.5 mt-auto">
                    <Link to={`/quick-practice/${set.id}`}>
                      <Button className="w-full bg-purple-600 hover:bg-purple-700 h-9 text-sm gap-1.5">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={async () => { await clearWrongAnswersForSet(set.id + 1000, set.questions); setRefreshKey(k => k + 1); }}
