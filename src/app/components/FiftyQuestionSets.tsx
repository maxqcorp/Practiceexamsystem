...
export default function FiftyQuestionSets() {
  ...
  const continueSet = examSets.find(set => {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Reset all DumpsGate Standard sets?</AlertDialogTitle>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2.5 mb-1">
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-indigo-100 text-indigo-700 border border-indigo-200">DumpsGate Standard</span>
          </div>
          <h1 className="text-xl font-semibold text-gray-900">DumpsGate Standard Practice</h1>
          <p className="text-sm text-gray-400 mt-0.5">{totalQuestions.toLocaleString()} questions · {examSets.length} sets</p>
        </div>
        ...
        {filteredSets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredSets.map(set => {
              ...
              return (
                <div key={set.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-3 hover:border-indigo-300 hover:shadow-sm transition-all">
                  {/* Set header */}
                  ...
                  <div className="flex flex-col gap-1.5 mt-auto">
                    <Link to={`/exam/${set.id}`}>
                      <Button className="w-full bg-indigo-600 hover:bg-indigo-700 h-9 text-sm gap-1.5">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={async () => { await clearWrongAnswersForSet(set.id + 0, set.questions); setRefreshKey(k => k + 1); }}
