...
export default function DavidYTExam() {
  ...
  useEffect(() => {
    if (examSet) {
      getExamSetProgress(examSet.id + 3000).then(savedProgress => {
  ...
  useEffect(() => {
    if (examSet && answers.size > 0) {
      saveExamSetProgress(examSet.id + 3000, answers);
    }
  }, [answers, examSet]);

  if (!examSet) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-100 p-6 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Set not found</h1>
          <Link to="/sets-davidyt">
            <Button>Back to David YT Sets</Button>
          </Link>
        </div>
      </div>
    );
  ...
  const handleReset = async () => {
    setAnswers(new Map());
    if (examSet) {
      await clearExamSetProgress(examSet.id + 3000);
    }
  };
      ...
      <Card key={question.id} className="mb-6">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span className="text-orange-600">{question.questionNumber}</span>
            {isSubmitted && (
              isCorrect ?
